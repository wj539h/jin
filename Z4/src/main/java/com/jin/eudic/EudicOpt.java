...
public class EudicOpt {
    ...
    public Map<String, String> categoryMap = null;
	...
	if (StringUtils.isNotEmpty(categoryId)) {
		...
		System.out.println(count + " 生词本 : " + categoryName + " --- " + response.getStatusLine() + " --- " + Thread.currentThread().getName());
		if (entity != null) {
		    Gson gson = new Gson();
		    Map<String, List> map = gson.fromJson(json, Map.class);
		    List<Map<String, String>> list = (List) map.get("data");
	...
	for (String key : keySet) {
	    ...
	    List<String> wordsInCat = findWordsInCategory(key, count++);
	    ...
	    public void run() {
	        List<String> wordsInCate = findWordsInCategory(key);
	        table.put(key, wordsInCate==null?new ArrayList<String>():wordsInCate);
	        if(keySet.size() == table.size()) {
    ...
    //{"msg":"添加分组成功","bookid":"132453104041475005","result":true}     "catename", "gogogo"  "oper": "insertcate"
    ...
    //{"msg":"分组名称更改成功","result":true}   "uuid", "0"   "oper", "changecatename"   "catename", "temp（默认）Jin"
    ...
    private String callCatPost(List<NameValuePair> paramList) {
	String result = null;
	CloseableHttpResponse response = EudicUtils.sendPost(URL_CAT_WORD_OPT, paramList);
	try {
	...
	if (!categoryMap.containsKey(catName)) {
	    ...
	    String wordsStr = String.join(STR_WORD_JOINNER, wordList.stream().map(String::valueOf).collect(Collectors.toList()));
