...
public class SimpleHttpServer implements Runnable {
	...
	while (true) {
		...
		if (client != null) {
			...
			if (resource.endsWith(".ico")) {
			    imgDownload("favicon.ico", client);
			    closeSocket(client);
			    continue;
			}
			...
			if (resource.endsWith(".jpg")) {
			    ...
			    out.println("你请求的页面含有图片:<img src='test.gif'><br>" + "<a href='test.gif'>手动点击打开test.gif图片文件.</a>");
			    out.println("<br>服务器不支持jpg格式图片，所以显示XX:" + "<img src='test.jpg'><br><a href='test.jpg'>" + "手动点击打开test.jpg，会跳转另一页面，并且服务返回为404错误</a><br>");
			    out.println(
				    "<form method=post action='/path?qryParm=POST URL查询参数' > POST 表单 " + "<input name=username value='用户'> " + "<input name=submit type=submit value=submit></form>");
			    out.println("<form method=get action='/path?qryParm=GET URL查询参数' > GET 表单 " + "<input name=username value='用户'> " + "<input name=submit type=submit value=submit></form>");

			    out.println("<form method=post action='/path?qryParm=POST URL查询参数'" + " enctype='multipart/form-data' >" + "文件上传  <input type='file' name=file1 ><br>" + "          "
				    + "<input type='file' name=file2 ><br>" + "          " + "<input name=username value='用户'> " + "<input name=submit type=submit value=submit></form>");
	...
	if (tmpStr.startsWith("Referer")) {//如果有Referer头时，使用UTF-8编码  
	    tmpStr = new String(tmpByteArr, encoding);
	}
	return tmpStr;
    }
    ...
    */
	...
	try {
	    ...
	    String aPath = fileToSend.getAbsolutePath();
	    String cPath = fileToSend.getCanonicalPath();
